...
import com.gchess.domain.model.*
...
class StandardChessRules : ChessRules {
    ...
    override fun legalMovesFor(position: ChessPosition): List<Move> {
        return position.getPiecesByColor(position.sideToMove)
    ...
    override fun isInCheck(position: ChessPosition, color: Color): Boolean {
        // Find the king of the given color
        val kingPosition = position.getPiecesByColor(color)
            .entries
        ...
        return threatenedSquares(position, opponentColor).contains(kingPosition)
    }

    override fun threatenedSquares(position: ChessPosition, by: Color): Set<Position> {
        return position.getPiecesByColor(by)
            .flatMap { (pos, piece) ->
    ...
    private fun generatePawnMoves(position: Position, piece: Piece, board: ChessPosition): List<Move> {
        ...
        val captureMoves = generatePawnCaptures(position, piece.color, direction, promotionRank, board)
    ...
    private fun generateRayMoves(
        ...
        generateSequence(1) { it + 1 }
            ...
            .filter { (_, targetPiece) -> targetPiece == null || targetPiece.color != piece.color }
    ...
    private fun generateCastlingMoves(position: Position, piece: Piece, board: ChessPosition): List<Move> {
        ...
        val threatenedSquares = threatenedSquares(board, opponentColor)

        if (piece.color == Color.WHITE) {
            // White kingside castling (petit roque)
            if (board.castlingRights.canCastleKingside(Color.WHITE) && position == Position.fromAlgebraic("e1")) {
            ...
            if (board.castlingRights.canCastleQueenside(Color.WHITE) && position == Position.fromAlgebraic("e1")) {
            ...
            if (board.castlingRights.canCastleKingside(Color.BLACK) && position == Position.fromAlgebraic("e8")) {
            ...
            if (board.castlingRights.canCastleQueenside(Color.BLACK) && position == Position.fromAlgebraic("e8")) {
    ...
    private fun wouldMoveCauseCheck(move: Move, board: ChessPosition): Boolean {
        ...
        val opponentColor = if (board.sideToMove == Color.WHITE) Color.BLACK else Color.WHITE
        ...
        if (movingPiece?.type == PieceType.KING) {
            ...
            val threatenedSquares = threatenedSquares(boardWithoutKing, opponentColor)
